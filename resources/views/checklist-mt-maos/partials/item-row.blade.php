...
@endphp
<div class="flex flex-col sm:grid sm:grid-cols-[28px_1fr_110px] lg:grid-cols-[28px_1fr_140px] items-start gap-2.5 sm:gap-3 py-4">
    <div class="flex items-center justify-between sm:block w-full sm:w-auto">
        ...
        <div class="flex sm:hidden justify-end gap-1.5" data-toggle-group>
            <input type="hidden" name="results[{{ $key }}]" value="{{ $current }}">
            <button type="button" title="Sesuai" data-val="ok"
                    class="tbtn flex h-9 px-3 items-center justify-center gap-1 rounded-lg border-2 text-xs font-bold transition {{ $current === 'ok' ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-slate-200 text-slate-500 bg-slate-50' }}">
                <i data-lucide="check" class="h-4 w-4"></i> Sesuai
            </button>
            <button type="button" title="Temuan" data-val="bad"
                    class="tbtn flex h-9 px-3 items-center justify-center gap-1 rounded-lg border-2 text-xs font-bold transition {{ $current === 'bad' ? 'border-brand-red bg-brand-red text-white' : 'border-slate-200 text-slate-500 bg-slate-50' }}">
    ...
    <div class="hidden sm:flex justify-end gap-1.5" data-toggle-group>
        <input type="hidden" name="results[{{ $key }}]" value="{{ $current }}">
        <button type="button" title="Sesuai" data-val="ok"
