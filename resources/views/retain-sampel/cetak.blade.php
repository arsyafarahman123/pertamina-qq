...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .slide-card { box-shadow: none !important; border: 1px solid #cbd5e1 !important; page-break-inside: avoid; }
        }
        .btn-action { transition: all .15s ease; }
        .btn-action:hover { transform: translateY(-1px); }
    </style>
</head>
<body class="bg-slate-900 py-6 px-4 sm:px-6 text-slate-800" x-data="{ modeTampil: 'rekap-semua' }" x-effect="if (window.lucide) $nextTick(() => lucide.createIcons())">
...
<div class="no-print mx-auto mb-6 flex max-w-7xl flex-wrap items-center justify-between gap-3 bg-slate-800/95 backdrop-blur p-4 rounded-2xl border border-slate-700 shadow-xl">
    <div class="flex items-center gap-3">
        ...
        <div class="hidden sm:flex items-center gap-1 bg-slate-900/80 p-1 rounded-xl border border-slate-700 text-xs font-bold text-slate-400">
            <button type="button" @click="modeTampil = 'rekap-semua'"
                ...
                <i data-lucide="layers" class="h-3.5 w-3.5"></i> Rekap Lengkap Harian (1 Lembar)
            </button>
    ...
    <div class="flex flex-wrap items-center gap-2">
        ...
        <button onclick="window.print()"
                class="btn-action inline-flex items-center gap-1.5 rounded-xl bg-red-600 px-4 py-2 text-xs font-bold text-white shadow hover:bg-red-500" style="background:#DA251D;">
            <i data-lucide="printer" class="h-3.5 w-3.5"></i> Cetak / Simpan PDF (1 Halaman)
        </button>
    </div>
</div>
...
<div class="mx-auto max-w-7xl">
    ...
    {{-- HANYA TAMPILKAN 1 SLIDE SESUAI PILIHAN (DEFAULT: REKAP LENGKAP HARIAN) --}}
